...
事实上这两种情况内核会有两种不同的处理方式，示例2这种情况的处理与我们上面的猜测相同，而示例1则有另外一种处理方式：PHP代码的编译是顺序的，示例1的情况编译到`echo my_class::A1`这行时首先会尝试检索下是否已经编译了my_class，如果能在CG(class_table)中找到，则进一步从类的contants_table查找对应的常量，找到的话则会复制其value替换常量，简单的讲就是类似C语言中的宏，编译的时候替换为实际的值了，而不是在运行时再去检索。
